...
module(AMAZON_SECURITYGROUP_SECURITYGROUP_TRANSFORMER, []).factory('awsSecurityGroupTransformer', function () {
  ...
  function addVpcNameToSecurityGroup(securityGroup: any) {
    return function (vpcs: any[]) {
      const matches = vpcs.filter(function (test: any) {
  ...
  function compress(securityGroups: any[]) {
    const grouped = groupBy(securityGroups, 'vpcId');
    Object.keys(grouped).forEach((vpcId) => {
      grouped[vpcId] = grouped[vpcId].map((g: any) => [g.name, g.id]);
    });
    return grouped;
  }

  function decompress(groupedGroups: any) {
    const flattened: any[] = [];
    Object.keys(groupedGroups).forEach((vpcId) => {
      groupedGroups[vpcId].forEach((g: any) => {
