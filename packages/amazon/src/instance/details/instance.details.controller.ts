...
interface IAWSInstanceDetailsScope extends IScope {
  isDisabled: boolean;
...
module(AMAZON_INSTANCE_DETAILS_INSTANCE_DETAILS_CONTROLLER, [UIROUTER_ANGULARJS]).controller('awsInstanceDetailsCtrl', [
  '$scope',
  '$state',
  'instance',
  'app',
  ...
  'overrides',
  function (
    $scope: IAWSInstanceDetailsScope,
    $state: any,
    instance: any,
    app: Application,
    moniker: any,
    ...
    function extractHealthMetrics(instance: any, latest: any) {
      if (app.isStandalone) {
        instance.health = latest.health;
      }

      instance.health = instance.health || [];
      const displayableMetrics = instance.health.filter(function (metric: any) {
        return metric.type !== 'Amazon' || metric.state !== 'Unknown';
      });

      if (!app.isStandalone) {
        const targetGroups = getAllTargetGroups(
          app.loadBalancers.data.filter(function (loadBalancer: any) {
    ...
    function retrieveInstance() {
      const extraData: any = {};
      let instanceSummary: any,
        loadBalancers: any,
        targetGroups: any,
        account: any,
        region: any,
        vpcId: any,
        serverGroupDisabled: any;
      if (!app.serverGroups) {
        ...
        if (!instanceSummary) {
          app.loadBalancers.data.some(function (loadBalancer: any) {
            return (
              loadBalancer.instances.some(function (possibleInstance: any) {
                if (possibleInstance.id === instance.instanceId) {
                  instanceSummary = possibleInstance;
              ...
              loadBalancer.targetGroups.some(function (targetGroup: any) {
                return targetGroup.instances.some(function (possibleInstance: any) {
                  if (possibleInstance.id === instance.instanceId) {
                    instanceSummary = possibleInstance;
          ...
          if (!instanceSummary) {
            app.loadBalancers.data.some(function (loadBalancer: any) {
              return (
                loadBalancer.serverGroups.some(function (serverGroup: any) {
                ...
                loadBalancer.targetGroups.some(function (targetGroup: any) {
                  targetGroup.serverGroups.some(function (serverGroup: any) {
                    if (!serverGroup.isDisabled) {
                      return false;
                    }
                    return serverGroup.instances.some(function (possibleInstance: any) {
      ...
      if (instanceSummary && account && region) {
        ...
        return InstanceReader.getInstanceDetails(account, region, instance.instanceId).then((details: any) => {
          if ($scope.$$destroyed) {
            return;
          }
          ...
          if ($scope.instance.networkInterfaces) {
            $scope.instance.ipv6Addresses = _.flatMap($scope.instance.networkInterfaces, (i: any) =>
              i.ipv6Addresses.map((a: any) => ({
                ip: a.ipv6Address,
            ...
            const permanentNetworkInterfaces = $scope.instance.networkInterfaces.filter(
              (f: any) => f.attachment.deleteOnTermination === false,
            );
            if (permanentNetworkInterfaces.length) {
              $scope.instance.permanentIps = permanentNetworkInterfaces.map((f: any) => f.privateIpAddress);
            }
          }
    ...
    this.canDeregisterFromLoadBalancer = function () {
      ...
      return healthMetrics.some(function (health: any) {
    ...
    this.canRegisterWithLoadBalancer = function () {
      ...
      const outOfService = healthMetrics.some(function (health: any) {
        return health.type === 'LoadBalancer' && health.state === 'OutOfService';
      });
      const hasLoadBalancerHealth = healthMetrics.some(function (health: any) {
        return health.type === 'LoadBalancer';
      });
    ...
    this.canRegisterWithTargetGroup = function () {
      ...
      const hasTargetGroupHealth = healthMetrics.some(function (health: any) {
    ...
    this.canRegisterWithDiscovery = function () {
      const instance = $scope.instance;
      const healthMetrics = instance.health || [];
      const discoveryHealth = healthMetrics.filter(function (health: any) {
    ...
    this.hasHealthState = function hasHealthState(healthProviderType: string, state: string) {
      const instance = $scope.instance;
      const healthMetrics = instance.health || [];
      return healthMetrics.some(function (health: any) {
    ...
    const constructInstanceActions = (instance: any) => {
